...
function getJandiWebhook(
  ...
  if (mainCategory === "ORDER") {
    ...
    if (subCategory === "CREATE") {
      if (receiver === "서울") return process.env.JANDI_KR!;
      return process.env.JANDI_VN!; // 대천, 베트남
    }

    // 주문서 결재 완료 단계
    if (subCategory === "APPROVED_SEOUL") return process.env.JANDI_KR!;
    if (
      subCategory === "APPROVED_DAECHEN" ||
      subCategory === "APPROVED_VIETNAM"
    )
      return process.env.JANDI_VN!;
  }

  // 2️⃣ 발주서 (사출 / 인쇄 / 메탈 전부 한국)
  if (mainCategory === "PURCHASE") {
    return process.env.JANDI_KR!;
  }
