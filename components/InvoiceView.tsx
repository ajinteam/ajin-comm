...
const InvoiceView: React.FC<InvoiceViewProps> = ({ sub, currentUser, setView, dataVersion }) => {
  ...
  const renderInvoiceForm = (isReadOnly: boolean = false, data?: InvoiceItem) => {
    ...
    return (
      <div className={`bg-white border-[1px] border-slate-300 shadow-2xl mx-auto p-4 md:p-12 min-h-[297mm] w-full max-w-[1400px] text-slate-800 font-gulim relative document-print-content text-left overflow-x-auto`}>
  ...
  if (activeInvoice) {
    ...
    return (
      <div className="py-4 md:py-8 bg-slate-200 min-h-screen">
        <div className="max-w-[1400px] mx-auto mb-4 md:mb-6 flex flex-col md:flex-row justify-between items-center px-4 no-print gap-4">
          <div className="flex gap-2">
            ...
            {(isMaster || (activeInvoice.isTemporary && (activeInvoice.authorId || '').toUpperCase() === (currentUser?.initials || '').toUpperCase())) && (
          ...
          {paginatedItems.length === 0 ? (<div className="col-span-full py-24 text-center text-slate-400 bg-white rounded-3xl border-4 border-dashed border-slate-100 text-lg font-medium italic">데이터가 없습니다.</div>) : (paginatedItems.map(inv => {
            ...
            return (
              <div key={inv.id} className="relative group">
                ...
                {(isMaster || (inv.isTemporary && (inv.authorId || '').toUpperCase() === (currentUser?.initials || '').toUpperCase())) && <button onClick={(e) => { e.stopPropagation(); setModal({ type: 'DELETE_FILE', message: '해당 송장 파일을 영구 삭제하시겠습니까? (복구 불가)', onConfirm: () => handleFileDelete(inv.id) }); }} className="absolute -top-2 -right-2 bg-red-600 text-white w-7 h-7 md:w-8 md:h-8 rounded-full shadow-lg hover:bg-red-700 flex items-center justify-center z-10"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg></button>}
              </div>
            );
          }))}
        </div>
