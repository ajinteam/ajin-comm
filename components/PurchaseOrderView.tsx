...
const PurchaseOrderView: React.FC<PurchaseOrderViewProps> = ({ sub, currentUser, setView, dataVersion }) => {
  ...
  const handleSaveVendor = () => {
    ...
    saveRecipient({
      id: `po-vendor-${newVendor.name}`,
      name: newVendor.name,
      tel: newVendor.tel,
      remark: newVendor.remarks,
      category: 'PO_RECIPIENT'
    });
  ...
  const handlePo2Submit = (isTemp: boolean = false) => {
    ...
    setView({ type: currentViewType, sub: targetStatus });
  ...
  const confirmReject = () => {
    ...
    if (targetItem) {
      sendJandiNotification('KR_PO', 'REJECT', `[${targetItem.recipient || '-'}] ${targetItem.title}`, targetItem.authorId, targetItem.date);
    }
    ...
    const updated = items.map(item => {
      if (item.id === id) {
        ...
        if (isComplete) {
            ...
            sendJandiNotification('KR_PO', 'COMPLETE', notifyTitle, item.authorId, item.date);
        } else {
            // 단계별 다음 결재자 호출
            if (stampType === 'design') {
                // 설계 승인 후 이사(무연)에게 요청
      ...
      <div className="space-y-8 py-12 animate-in fade-in zoom-in duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto px-4 mt-12">
          {poTypes.map((po) => {
            ...
            return (
              <button key={po.id} onClick={() => setView({ type: currentViewType, sub: po.id })} className="group bg-white p-10 rounded-[2.5rem] border-2 border-slate-100 hover:shadow-2xl transition-all duration-300 flex flex-col items-center text-center relative overflow-hidden">
    ...
    return (
      <div className="space-y-6">
        ...
        {isVendorManagerOpen && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 no-print">
            <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-2xl shadow-2xl animate-in fade-in zoom-in duration-300">
              ...
              <div className="max-h-64 overflow-y-auto space-y-2 custom-scrollbar">{vendors.map(v => (<div key={v.name} className="flex justify-between items-center p-4 bg-white border border-slate-100 rounded-2xl hover:bg-amber-50 transition-colors"><div><p className="font-black text-black">{v.name}</p><p className="text-xs text-slate-400">{v.tel} | {v.remarks}</p></div><div className="flex gap-2"><button onClick={() => setNewVendor(v)} className="text-xs font-bold text-amber-600 hover:underline">편집</button><button onClick={() => { const filtered = vendors.filter(vendor => vendor.name !== v.name); setVendors(filtered); localStorage.setItem('ajin_vendors', JSON.stringify(filtered)); deleteRecipient(`po-vendor-${v.name}`); }} className="text-xs font-bold text-red-500 hover:underline">삭제</button></div></div>))}</div>
            </div>
          </div>
        )}
        {po1Selection && (
        ...
        <div className="py-4 md:py-8 bg-slate-200 min-h-screen overflow-x-auto overflow-y-auto">
          <div className="bg-white border-[1px] border-slate-200 shadow-2xl mx-auto p-4 md:p-12 min-h-[297mm] w-full max-w-full md:max-w-[1600px] text-black font-gulim text-left overflow-x-auto animate-in slide-in-from-bottom-8 duration-500">
  ...
  if (activeItem) {
    ...
    return (
      <div className="py-8 bg-slate-200 min-h-screen">
        <div className="max-w-[1600px] mx-auto mb-6 flex flex-wrap justify-between items-center px-4 no-print gap-4">
...
<div className="flex flex-wrap gap-3">
  ...
  <button onClick={handlePrint} className="bg-blue-600 text-white px-8 py-2.5 rounded-xl font-black shadow-lg hover:bg-blue-700 text-xs">
    PDF 저장 / 인쇄
        ...
        <div className="bg-white border-[1px] border-slate-200 shadow-2xl mx-auto p-4 md:p-12 min-h-[297mm] w-full max-w-full md:max-w-[1600px] text-black font-gulim text-left overflow-x-auto document-print-content">
          <div className="min-w-[800px] md:min-w-0">
            {isPOForm ? (
              <>
                {false ? (
              ...
              {activeItem.status === PurchaseOrderSubCategory.APPROVED && !stamps.final && (
                <div className="mt-12 flex justify-center no-print">
                  <button 
                    ...
                    className="px-20 py-4 bg-emerald-600 text-white rounded-2xl font-black shadow-xl hover:bg-emerald-700 transition-all active:scale-95 text-lg flex items-center gap-3"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
