  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-start">
        <div>
          <h2 className="text-3xl font-extrabold text-slate-900 mb-2">대시보드</h2>
          <p className="text-slate-500">AJIN COMMUNICATIONS 업무 관리 시스템에 오신 것을 환영합니다.</p>
        </div>
        {isMaster && (
          <div className="animate-pulse bg-amber-500 text-white px-4 py-2 rounded-xl text-xs font-black shadow-lg shadow-amber-200 uppercase tracking-widest">
            Master Mode Active
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <button 
          ...
          className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:border-blue-500 hover:shadow-md transition-all text-left"
        >
          <p className="text-slate-500 text-sm font-medium mb-1">결재 대기</p>
          <p className={`text-3xl font-bold text-slate-900 ${pendingOrdersCount > 0 ? 'animate-blink text-red-600' : ''}`}>처리 대기중</p>
          <p className="text-blue-600 text-xs mt-4 font-semibold uppercase tracking-wider">상세보기 →</p>
        </button>
        ...
        <button 
          ...
          className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:border-blue-500 hover:shadow-md transition-all text-left"
        >
          <p className="text-slate-500 text-sm font-medium mb-1">결재 완료</p>
          <p className={`text-3xl font-bold text-slate-900 ${approvedOrdersCount > 0 ? 'animate-blink text-blue-600' : ''}`}>데이터 확인</p>
          <p className="text-blue-600 text-xs mt-4 font-semibold uppercase tracking-wider">상세보기 →</p>
        </button>
        ...
        <button 
          ...
          className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:border-blue-500 hover:shadow-md transition-all text-left"
        >
          <p className="text-slate-500 text-sm font-medium mb-1">송장 완료</p>
          <p className={`text-3xl font-bold text-slate-900 ${completedInvoicesCount > 0 ? 'animate-blink text-emerald-600' : ''}`}>
            {completedInvoicesCount > 0 ? '확인 필요' : '데이터 확인'}
          </p>
          <p className="text-blue-600 text-xs mt-4 font-semibold uppercase tracking-wider">상세보기 →</p>
        </button>

        <div className={`p-6 rounded-2xl shadow-sm text-white flex flex-col justify-between ${isMaster ? 'bg-gradient-to-br from-amber-600 to-amber-900' : 'bg-slate-900'}`}>
          <div>
            <p className="text-white/60 text-sm mb-1">현재 로그인 계정</p>
            <p className="text-xl font-mono font-bold tracking-tight flex items-center gap-2 truncate">
              {user.loginId}
              {isMaster && <span className="text-[10px] bg-white/20 px-2 py-0.5 rounded backdrop-blur-sm">MASTER</span>}
            </p>
          </div>
          <div className="mt-4 pt-4 border-t border-white/10 flex justify-between items-center">
            <span className="text-[10px] text-white/40">접속: {new Date().toLocaleDateString()}</span>
            <span className="text-[9px] bg-green-500/20 text-green-400 px-2 py-1 rounded uppercase font-bold tracking-tighter">Connected</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        {isMaster && (
          <div className="mb-8 p-4 bg-slate-50 rounded-xl border border-slate-200">
            <p className="text-xs font-bold text-slate-500 mb-2 uppercase tracking-tight">공지 관리 (마스터 권한)</p>
        ...
        <ul className="space-y-2">
          {announcements.map(n => (
            <li key={n.id} className="group flex items-center justify-between py-3 px-4 border border-transparent hover:border-slate-100 hover:bg-slate-50 rounded-xl transition-all">
              <div className="flex items-center gap-4">
                <span className="text-slate-700 font-medium">{n.content}</span>
                {n.isNew && <span className="text-[10px] bg-amber-100 text-amber-600 px-2 py-0.5 rounded-full font-black">NEW</span>}
              </div>
              <div className="flex items-center gap-4">
                <span className="text-xs text-slate-400 tabular-nums">{n.date}</span>
                {isMaster && (
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => startEdit(n)} className="text-xs font-bold text-blue-600 hover:underline">수정</button>
                    <button onClick={() => handleDeleteNotice(n.id)} className="text-xs font-bold text-red-500 hover:underline">삭제</button>
                  </div>
                )}
              </div>
            </li>
